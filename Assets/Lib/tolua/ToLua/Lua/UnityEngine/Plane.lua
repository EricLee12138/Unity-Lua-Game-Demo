...
function Plane.New(normal, d)
	return setmetatable({normal = Plane._retainNew(nil, normal:Normalize()), distance = d}, Plane)	
end

------- be typesys external type -------
Plane._overrideNew = Plane.New
Plane._retainNew = function(old_v, new_v)
	return new_v
end
Plane.New = function(in_normal, distance)
	return Plane._overrideNew(in_normal, distance)
end
----------------------------------------
...
function Plane:SetNormalAndPosition(inNormal, inPoint)    
    self.normal = Plane._retainNew(self.normal, inNormal:Normalize())
    self.distance = -Vector3.Dot(inNormal, inPoint)
end    

function Plane:Set3Points(a, b, c)    
    self.normal = Plane._retainNew(self.normal, Vector3.Normalize(Vector3.Cross(b - a, c - a)))
