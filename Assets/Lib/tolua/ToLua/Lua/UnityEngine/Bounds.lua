...
function Bounds.New(center, size)	
	return setmetatable({center = Bounds._retainNew(nil, center), extents = Bounds._retainNew(nil, size * 0.5)}, Bounds)		
end

------- be typesys external type -------
Bounds._overrideNew = Bounds.New
Bounds._retainNew = function(old_v, new_v)
	return new_v
end
Bounds.New = function(center, size)
	return Bounds._overrideNew(center, size)
end
----------------------------------------
...
function Bounds:SetCenter(value)
	self.center = Bounds._retainNew(self.center, value)
end
...
function Bounds:SetSize(value)
	self.extents = Bounds._retainNew(self.extents, value * 0.5)
end
...
function Bounds:SetMinMax(min, max)
	self.extents = Bounds._retainNew(self.extents, (max - min) * 0.5)
	self.center = Bounds._retainNew(self.center, min + self.extents)
end
...
function Bounds:Destroy()
	self.extents	= Bounds._retainNew(self.extents, nil)
	self.center	= Bounds._retainNew(self.center, nil)
end
