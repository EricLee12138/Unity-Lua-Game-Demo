...
function Touch.New(fingerId, position, rawPosition, deltaPosition, deltaTime, tapCount, phase)	
	return setmetatable({
		...
		position = Touch._retainNew(nil, position or Vector2.zero), 
		rawPosition = Touch._retainNew(nil, rawPosition or Vector2.zero), 
		deltaPosition = Touch._retainNew(nil, deltaPosition or Vector2.zero), 
		deltaTime = deltaTime or 0, 
		tapCount = tapCount or 0, 
		phase = phase or 0}, Touch)	
end

------- be typesys external type -------
Touch._overrideNew = Touch.New
Touch._retainNew = function(old_v, new_v)
	return new_v
end
Touch.New = function(fingerId, position, rawPosition, deltaPosition, deltaTime, tapCount, phase)
	return Touch._overrideNew(fingerId, position, rawPosition, deltaPosition, deltaTime, tapCount, phase)
end
----------------------------------------

function Touch:Init(fingerId, position, rawPosition, deltaPosition, deltaTime, tapCount, phase)
	self.fingerId = fingerId
	self.position = Touch._retainNew(self.position, position)
	self.rawPosition = Touch._retainNew(rawPosition)
	self.deltaPosition = Touch._retainNew(deltaPosition)
...
function Touch:Destroy()
	self.position 		= Touch._retainNew(self.position, nil)
	self.rawPosition	= Touch._retainNew(self.rawPosition, nil)
	self.deltaPosition 	= Touch._retainNew(self.deltaPosition, nil)	
end
